...
export const MessageScreen = ({ route }: Props) => {
    ...
    const getIdChat=async()=>{
        ...
        const msj=firestore().collection('Messages').orderBy('timestamp','asc').where('idChat','==',idChat.current).onSnapshot((query)=>{
         ...
         //   console.log(query)
          ...
          ToastAndroid.show('llego',ToastAndroid.LONG);
    ...
    useEffect(() => {
        const user=firestore().collection('users').doc(idUser).get().then(resp=>{
    ...
    const createChatWithMessage = () => {
        const data: any[] = [idUser + state.uid, state.uid + idUser];
        firestore().collection('chats').where('ids', 'in', data).get().then(resp => {
            if (resp.size > 0) {
        ...
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <View style={{ borderBottomWidth: 1, borderBottomColor: '#ECEEEF' }}>
                <View style={{ flexDirection: 'row', marginHorizontal: 10, marginVertical: 5 }}>
                    ...
                    <Image source={{ uri: current }} style={{ width: 40, height: 40, borderRadius: 100,marginLeft:10 }} />
                    <Text style={{ marginHorizontal: 5, fontSize: 16 ,marginTop:5,color:'black'}}>{users!.displayName}</Text>
                    <View style={{ flex: 1 }} />
                ...
                <FlatList
                    ...
                    renderItem={({ index, item }) => (
                        (item.idEmisor == state.uid) ?
                            <CardMessage color={Primary} timestamp='' message={item.message} align='flex-end' />
                            :
                            <CardMessage color='white' timestamp='' message={item.message} align='flex-start' />
                    )}
                />
            ...
            <View style={{ paddingVertical: 9, borderRadius: 10, marginHorizontal: 10, flexDirection: 'row', paddingHorizontal: 1, justifyContent: 'center', alignItems: 'center' }}>
                <TextInput onChangeText={(txt) => onChange(txt, 'message')} placeholder='Escribe un mensaje' style={{ paddingLeft: 10, backgroundColor: '#ECEEEF', flex: 1, borderRadius: 100 }} />
